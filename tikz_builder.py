"""
Construit dynamiquement un schéma TikZ (architecture réseau) à partir des
zones / nœuds / flux / équipements réseau définis dans le fichier de
configuration client.

Ce module ne dépend d'aucun template : il produit directement une chaîne
LaTeX valide ("\\begin{tikzpicture}...\\end{tikzpicture}") qui est ensuite
injectée telle quelle dans le document via la variable de contexte
`diagram_tikz`. Les couleurs par catégorie de flux (\\definecolor) doivent
avoir été définies AVANT ce bloc (voir generate_dat.py / preamble).
"""
import re
